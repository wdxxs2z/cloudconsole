			...
			<div class="span11">
				<div class="content">
					<div class="content-header">
						<h2>
							<i class="icofont-home"></i> 编辑云安全组  <small> Edit CloudSecurityGroup</small>
						</h2>
					</div>
						...
						<div class="span12">
                           	...
                           	<div class="tab-content">
                           		...
                           		<table class="table table-hover table-bordered">
                           			<tr>
                           				<td class="span2"><strong>安全组名称</strong></td>
                           				<td><a href="#" id="name" data-url="securityGroupOperation"
																data-value="<%=cloudSecurityGroup.getName()%>"
																data-original-title="input new name" data-type="text"
																data-pk="<%=cloudSecurityGroup.getName()%>"><%=cloudSecurityGroup.getName()%></a>
										</td>
                           			</tr>
                           			<tr>
                           			<%
                           				String stagingData = "";
                           				Boolean[] isStagings = {true,false};
                           				for (Boolean isStaging : isStagings){
                           					String tmp = "{value:" + isStaging + ",text:'" + isStaging + "'}";
											if (StringUtils.isNotBlank(stagingData)) {
												stagingData = stagingData + "," + tmp;
											} else {
												stagingData = tmp;
											}
                           				}
                           			%>
                           				<td class="span2"><strong>编译安全组</strong></td>
                           				<td><a id="staging" href="#" data-url="securityGroupOperation"
																data-value="<%=cloudSecurityGroup.getStaging_default() %>" data-type="select"
																data-source="[<%=stagingData%>]"
																data-pk="<%=cloudSecurityGroup.getName() %>"
																data-original-title="Select Staging_Security"><%=cloudSecurityGroup.getStaging_default() %></a></td>
                           			</tr>
                           			<tr>
                           			<%
                           				String RunningData = "";
                           				Boolean[] isRunnings = {true,false};
                           				for (Boolean isRunning : isRunnings){
                           					String tmp = "{value:" + isRunning + ",text:'" + isRunning + "'}";
											if (StringUtils.isNotBlank(RunningData)) {
												RunningData = RunningData + "," + tmp;
											} else {
												RunningData = tmp;
											}
                           				}
                           			%>
                           				<td class="span2"><strong>编译安全组</strong></td>
                           				<td><a id="running" href="#" data-url="securityGroupOperation"
																data-value="<%=cloudSecurityGroup.getRunning_default() %>" data-type="select"
																data-source="[<%=RunningData%>]"
																data-pk="<%=cloudSecurityGroup.getName() %>"
																data-original-title="Select Running_security"><%=cloudSecurityGroup.getRunning_default() %></a></td>
                           			</tr>
                           			<tr>
                           			<%
                           				String rulesJson = JsonUtil.convertToJson(cloudSecurityGroup.getRules());
                           			%>
                           				<td class="span2"><strong>安全组规则</strong></td>
                           				<td><a href="#" id="rules" data-url="securityGroupOperation"
																data-value="请复制列表规则并修改"
																data-original-title="Edit your new rules" data-type="textarea"
																data-pk="<%=cloudSecurityGroup.getName()%>"><%=rulesJson%></a>
										</td>
                           			</tr>
                           		</table>
                           		</div>
